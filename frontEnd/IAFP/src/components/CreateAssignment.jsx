...
function CreateAssignment() {
  const [formData, setFormData] = useState({
    ...
    description: "",
    department: "",
    questions: [{ questionText: "", expectedAnswer: "", marks: "" }],
    deadline: "",
  });
  ...
  const addQuestion = () => {
    setFormData({
      ...formData,
      questions: [...formData.questions, { questionText: "", expectedAnswer: "", marks: "" }],
    });
  };
    ...
    <div>
      ...
      <div className='form_CreateAssignment'>
        <form onSubmit={handleSubmit}>
          ...
          <input type="text" id="year" name="year" placeholder="Year" value={formData.year} onChange={handleChange} required />
          ...
          {formData.questions.map((q, index) => (
            <div key={index} style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ddd', borderRadius: '5px' }}>
              ...
              <label htmlFor={`expectedAnswer-${index}`}>Expected Answer (Optional - for grading):</label>
              <textarea
                id={`expectedAnswer-${index}`}
                name="expectedAnswer"
                placeholder="Enter expected answer for comparison during grading (not visible to students)"
              ...
              <input
                ...
                required
              />
              <button type="button" onClick={() => deleteQuestion(index)} style={{ marginTop: '10px' }}>Delete Question</button>
            </div>
          ))}
